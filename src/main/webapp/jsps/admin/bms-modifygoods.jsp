...
<%
    ...
    Goods goods = (Goods) request.getAttribute("modifyGoodsData");
%>
<head>
    <meta charset="UTF-8">
...
<body>
    ...
    <form action="${ pageContext.request.contextPath }/admin/GoodsServlet" method="post">
    	<table width="500px;">
	    	...
	    	<tr>
	    		<td>商品名称：</td>
	    		<td><input type="text" name="name" style="width: 200px" value="<%=goods.getName()%>"></td>
	    	<tr>
	    	<tr>
	    		<td>商品描述：</td>
	    		<td><textarea rows="4" cols="40" name="description"><%=goods.getDescription()%></textarea></td>
            </tr>
            <tr>
	    		<td>商品价格：</td>
	    		<td><input type="text" name="price" style="width: 200px" value="<%=goods.getPrice()%>"></td>
	    	<tr>
            <tr>
                <td>所属类别：</td>
                <td>
                    <select name="category_id">
                        ...
                        if (categories.size() > 0)
                            for (Category category : categories) {
                                if (category.getID().equals(goods.getCategory().getID())) {
                        %>
